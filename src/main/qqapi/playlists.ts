...
function summaryOf(p: any): QqPlaylistDTO | null {
  ...
  return {
    ...
    cover: p?.picurl ?? p?.picUrl ?? p?.bigpicUrl ?? p?.coverPicUrl ?? p?.albumPicUrl ?? '',
...
export async function getFavPlaylists(client: QqClient, euin: string): Promise<QqPlaylistDTO[]> {
  const data = (await client.postMusicu(
    {
  ...
  return (data?.v_list ?? []).map(summaryOf).filter((p: QqPlaylistDTO | null): p is QqPlaylistDTO => p !== null)
}
...
export async function getDissTracksPage(
  ...
  const total = Number(data?.total ?? opts.songBegin + tracks.length)

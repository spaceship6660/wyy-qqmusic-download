...
| 扫码登录（真实，2 次） | ⚠️ 失败 → **已修复（2026-09-04 归因更正）** | 手机 QQ 扫码并确认成功后，check_sig 步拿不到 p_skey。**根因非服务端风控，是本实现移植缺陷**：`redirect:'manual'` 只收首跳 Set-Cookie，而 p_skey 由 check_sig 重定向链的后续跳（login_jump）种下——Spica 的 urllib opener 跟随整链、逐跳累积所以能成。修复：`fetchChain` 跟随重定向链逐跳合并 Set-Cookie（commit b6b4569）+ 302/303 跟跳时 POST 降 GET 防落地页 405（commit f649af6）。**待用户真实扫码复验** |

### 1.4 冒烟发现并修复的缺陷（登录，2026-09-04 追认）

**check_sig 重定向链未跟随 → p_skey 丢失（登录失败根因）**：见上表。修复后单测防回归锚：p_skey 只落在第 2 跳的完整登录流（首跳裸 302），去掉跟链逻辑必挂；authorize 302 后方法降 GET 亦有独立用例。全量 80 用例绿。
...
1. **匿名下载被服务端收紧**（2026-09-02 实测）：`CgiGetVkey` 匿名返回空 purl，免费歌也要登录态。影响 M2「匿名下载」验收点，需用户登录态实测确认；若登录后仍空 purl 属服务端收紧常态，则「匿名下载」标记为不可用、全链路走登录态。

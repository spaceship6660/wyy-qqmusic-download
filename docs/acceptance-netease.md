...
4. **网易云扫码登录为「开窗抓 cookie」薄实现**（`src/main/neteaseAuth.ts`，Electron session API，非主应用逻辑），真实性依赖用户扫码实测（清单项 1）；MUSIC_U 凭证落盘明文于**仓库外的用户数据目录**（`app.getPath('userData')`，与 P1 QQ 侧 `qqmusic_cookie.json` 同纪律：不入库、不打印）。
